...
import {getCountries, getJobs, updateEmployee} from './_requests'
...
function EditEmployeeModal(props) {
  ...
  const [imageBg, setImageBg] = useState(userDetails.image)
  ...
  const [initialValues, setInitialValues] = useState({
    ...
    country_iso: userDetails.country.iso,
    job_id: userDetails.job.id,
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, {setStatus, resetForm, setFieldValue}) => {
      ...
      try {
        await updateEmployee(formData, userDetails.id).then((res) => {
          resetForm()
