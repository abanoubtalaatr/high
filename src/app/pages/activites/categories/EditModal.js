...
import { updateActivityCategory, updateType } from '../_requests'
...
function EditModal(props) {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { setStatus, resetForm }) => {
      ...
      try {
        await updateActivityCategory(formData, itemDetails.id).then((res) => {
          setAlertType('success')
          resetForm()
          // setStatus(res.data.message)
          setLoading(false)
          setUpdateImageStatus(false)
          onCompleteHideHandler()
        })
      } catch (error) {
        setAlertType('danger')
