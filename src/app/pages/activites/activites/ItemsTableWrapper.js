...
import {getActivities, getActivityCategory} from '../_requests'
...
function ItemsTableWrapper(props) {
  ...
  const breadCrumbs = [
    ...
    {
      title: intl.formatMessage({id: 'ACTIVITIES_CATEGORIES'}),
  ...
  useEffect(() => {
    getActivityCategory(catId)
      .then((res) => {
        const title = res.data.data.name + ' ' + intl.formatMessage({id: 'ACTIVITIES'})
        setPageTitle(title)
      })
      .catch((err) => {
        setPageTitle(intl.formatMessage({id: 'ACTIVITIES'}))
        // setErrorMessage(err.response.data.message)
      })
  }, [])
  const getitemssHandler = () => {
    getActivities(parms, catId)
      .then((res) => {
