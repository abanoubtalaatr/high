...
import { getTax, updateTax } from './_requests'
...
function EditModal(props) {
  ...
  useEffect(() => {
    try {
      getTax(codeId, location, itemId).then((res) => {
        const newData = res.data.data
        console.log(newData);
        setInitialValues({
          tax_name: newData.name,
          tax: newData.tax,
  ...
  const statusDefaultValue = () => {
    ...
    formik.setFieldValue('active', defaultValue.value)
  }
  useEffect(() => {
    statusDefaultValue()
  }, [initialValues])
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { setStatus, resetForm, setFieldValue }) => {
      setLoading(true)
      try {
        await updateTax(values.tax_name, values.tax, values.active, codeId, location, itemId).then(
          (res) => {
  ...
  return (
    <Modal show={show} onHide={onHideHandler} backdrop='static' keyboard={false}>
      <Modal.Header closeButton>
        <Modal.Title>edit {initialValues.tax_name}</Modal.Title>
      </Modal.Header>
      <Modal.Body>
