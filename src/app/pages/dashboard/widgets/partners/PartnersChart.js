...
function PartnersChart(props) {
  const { partnersStatistics, errorMessage, filter } = props
  ...
  const [chartActiveData, setChartActiveData] = useState([])
  ...
  const activeData = []
  const chartHandler = () => {
    if (Object.values(partnersStatistics).length !== 0) {
      for (const [key, value] of Object.entries(partnersStatistics.partners)) {
        lab.push(key)
        dat.push(value)
      }
    }
  ...
  const chartActiveHandler = () => {
    if (Object.values(partnersStatistics).length !== 0) {
      for (const [key, value] of Object.entries(partnersStatistics.active)) {
    ...
    datasets: [
      {
        label: 'partners',
        data: chartData,
        borderColor: '(255, 205, 86, 1)',
        backgroundColor: '(255, 205, 86, 1)',
        pointStyle: 'circle',
        pointRadius: 10,
        pointHoverRadius: 15,
      },
    ...
    <div className='card card-lg-stretch py-5'>
      ...
      <div className='card-header border-0 px-5'>
        <div className='card-title align-items-start flex-column col-md-8 m-0'>
          <h3 className='card-label fw-bold fs-3 m-0 text-gray-700'>partners statistics</h3>
          <small className='text-muted mt-1 fw-semibold fs-7'>number of registered partners.</small>
        </div>
      </div>
      {/* end::Header */}
      {/* begin::Body */}
