        ...
        <tbody>
          {branches.map((branch, index) => (
            <React.Fragment key={branch.id}>
              <tr className='expand_row cursor-pointer' onClick={() => handleRowClick(index)}>
                <td className='px-0'>
                  <KTIcon
                ...
                <td className='text-center'>
                  <span onClick={() => handleIconClick(branch.id)} className='btn'>
                    <KTIcon iconName='eye' className='fs-3' />
                  </span>
                  <Link
                    to={`/partners/profile/${userId}/branches/${branch.id}/translation`}
                    state={branch}
                  >
