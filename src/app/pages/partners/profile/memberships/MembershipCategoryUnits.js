...
import MembershipCategoryUnitsTable from './MembershipCategoryUnitsTable'

function MembershipCategoryUnits() {
  ...
  const [parms, setParms] = useState({
    status: '',
    search: '',
    membership_category_id: itemId || 0,
    page: 1,
    ...
    <div className='card mb-10'>
      ...
      <div className='card-header border-0 pt-5 align-items-end'>
        <h5 className='card-title align-items-start flex-column'>
          <span className='mb-3 fw-bolder'>
            {intl.formatMessage({id: 'UNITS'})} ({propsData.titleName})
          </span>
        </h5>
        <div className='card-toolbar'>
          <Link to={profilePath + '/memberships/categories'} className='btn btn-light btn-sm btn-flex fw-bold'>
      ...
      <div className='card-body py-3'>
        {!apiRespone || isLoaded ? (
          ...
          <MembershipCategoryUnitsTable items={items} />
        )}
      </div>
      {/* begin::Body */}
...
export default MembershipCategoryUnits
