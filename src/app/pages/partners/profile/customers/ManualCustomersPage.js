...
import {getCustomers, getEmployees} from '../../_requests'
import Select from 'react-select'
import {useIntl} from 'react-intl'
import ManualCustomersTable from './ManualCustomersTable'
import Pagination from '../../../../components/pagination/Pagination'
import Spinner from '../../../../components/spinner/Spinner'

function ManualCustomersPage() {
  const intl = useIntl()
    ...
    <div className='card'>
      ...
      <div className='card-header border-0 pt-5 align-items-end'>
        <div className=''>
          ...
          <div className='card-toolbar gap-3'>
            <Link
              to={`/partners/profile/${userId}/customers/high-five`}
              className='btn btn-light btn-sm'
            >
              high five
            </Link>
            <span className='btn btn-bg-dark btn-text-white btn-sm'>manual</span>
          </div>
        </div>
        <div>
          <div className='card-toolbar gap-3'>
      ...
      <div className='card-body py-3'>
        {isLoaded ? (
          ...
          <ManualCustomersTable items={items} />
        )}
      </div>
      {/* begin::Footer */}
