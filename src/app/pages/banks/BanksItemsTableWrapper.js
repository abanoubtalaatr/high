...
function BanksItemsTableWrapper(props) {
  ...
  const [isBanksDisabled, setIsBanksDisabled] = useState(false)
  const [banksOptions, setBanksOptions] = useState([])
  const [bankChoice, setBankChoice] = useState(DefOptions[0])
  ...
  const onChangeHandler = (e) => {
    setBankChoice(e)
    setParms({ ...parms, name: e.value })
  }
  // get items
  const getJobsHandler = () => {
    setIsBanksLoading(true)
    setIsBanksDisabled(true)
    getBanks(parms)
      .then((res) => {
...
export default BanksItemsTableWrapper
