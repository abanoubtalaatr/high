...
import { getAgeGroup, updateAgeGroup } from '../_requests'
...
function EditModal(props) {
  ...
  const formSchema = Yup.object().shape({
    ...
    starting_age: Yup.string()
      .required('this field is required')
      .min(1, 'Minimum 3 symbols')
      .max(10, 'Maximum 50 symbols')
      .matches(numberRegExp, 'number is not valid'),
    ending_age: Yup.string()
      .required('this field is required')
      .min(1, 'Minimum 3 symbols')
      .max(10, 'Maximum 50 symbols')
      .matches(numberRegExp, 'number is not valid'),
    description: Yup.string().min(1, 'Minimum 3 symbols').max(150, 'Maximum 50 symbols'),
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { setStatus, resetForm, setFieldValue }) => {
      ...
      formData.append('starting_age', values.starting_age)
      formData.append('end_age', values.ending_age)
      formData.append('description', values.description)
      formData.append('active', values.active)
      try {
        await updateAgeGroup(formData, itemDetails.id).then((res) => {
  ...
  return (
    <Modal show={show} onHide={onHide} backdrop='static' keyboard={false}>
      <Modal.Header closeButton>
        <Modal.Title>edit age group</Modal.Title>
      </Modal.Header>
      <Modal.Body>
        <form onSubmit={formik.handleSubmit} encType='multipart/form-data'>
          ...
          {/* age range */}
          <div className='row mb-5'>
            <label className='col-sm-3 form-label fw-bold'>age range:</label>
            <div className='col-sm-9'>
              <div className='row'>
                <div className='col-sm-6'>
                  <input
                    ...
                    min='1'
                    autoComplete='off'
                    {...formik.getFieldProps('starting_age')}
                    className={clsx('form-control form-control-solid', {
                      'is-invalid': formik.touched.starting_age && formik.errors.starting_age,
                    })}
                    placeholder='start'
                  />
                  {formik.touched.starting_age && formik.errors.starting_age && (
                    <div className='fv-plugins-message-container'>
                      <div className='fv-help-block'>
                        <span role='alert'>{formik.errors.starting_age}</span>
                      </div>
                    </div>
                  )}
                </div>
                <div className='col-sm-6'>
                  <input
                    ...
                    min='1'
                    autoComplete='off'
                    {...formik.getFieldProps('ending_age')}
                    className={clsx('form-control form-control-solid', {
                      'is-invalid': formik.touched.ending_age && formik.errors.ending_age,
                    })}
                    placeholder='end'
                  />
                  {formik.touched.ending_age && formik.errors.ending_age && (
                    <div className='fv-plugins-message-container'>
                      <div className='fv-help-block'>
                        <span role='alert'>{formik.errors.ending_age}</span>
                      </div>
                    </div>
                  )}
                </div>
