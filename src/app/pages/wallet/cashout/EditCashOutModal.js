...
import { updateWalletCashOut } from '../_requests'

function EditCashOutModal(props) {
  const {show, onHide, itemDetails, onComplete} = props
  ...
  const initialData = {
    ...
    action_reason: itemDetails.refundReason,
  }
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, {setStatus, resetForm, setFieldValue}) => {
      setLoading(true)
      try {
        await updateWalletCashOut(values.status, values.action_reason, itemDetails.userId, itemDetails.itemId).then((res) => {
