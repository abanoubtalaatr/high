...
import { getHelpCenter, getHelpCenterChildren } from '../_requests'
...
function ChildrenTableWrapper(props) {
  const intl = useIntl()
  const { refreshTable, startRefreshTable, stopRefresh } = props
  const location = useLocation()
  const parentDetails = location.state
  ...
  const getItemsHandler = () => {
    getHelpCenterChildren(parms, parentDetails.parentId)
      .then((res) => {
  ...
  return (
    <>
      <PageTitle breadcrumbs={breadCrumbs}>{parentDetails.parentTitle}</PageTitle>
      <div className='card'>
        ...
        <div className='card-body py-3'>
          {isLoaded ? (
            ...
            <ChildrenTable items={items} parentID={parentDetails.parentId} onComplete={completeHandler} />
          )}
        </div>
...
export default ChildrenTableWrapper
